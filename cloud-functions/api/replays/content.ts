...
import { DATA_STORE_NAME } from "../../_shared/constants";
...
export async function replayContentResponse(
  ...
  try {
    ...
    if (capability.expiresAt <= nowSeconds) {
      throw new HttpError(410, "这个分享链接已经过期");
    }

    const body = await store.get(capability.key, {
      type: "arrayBuffer",
      consistency: "strong",
    });
    ...
    return new Response(body, {
      headers: {
        ...
        "X-Replay-Expires-At": new Date(
          capability.expiresAt * 1000,
        ).toISOString(),
...
export async function onRequestGet(context: FunctionContext) {
  return replayContentResponse(context.request, getStore(DATA_STORE_NAME));
}
