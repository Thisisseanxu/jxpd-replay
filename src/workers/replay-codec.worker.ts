  ...
  try {
    while (iterations++ < 10_000) {
      ...
      if (outputOffset + result.buf.length > output.length) {
        throw new Error('回放解压后超过声明长度或安全上限');
      }
...
workerScope.onmessage = async (event: MessageEvent<CodecRequest>) => {
  ...
  try {
    ...
    if (event.data.operation === 'encode') {
      ...
      const packed = packReplayLossless(original);
      const compressed = brotli.compress(packed, { quality: 11 });
      ...
      workerScope.postMessage({ id, ok: true, bytes: container.buffer }, [
        container.buffer,
      ]);
      return;
    }
    ...
    const packed = decompressLimited(
      brotli,
      container.compressed,
      container.packedLength,
    );
    const output = unpackReplayLossless(packed, container.originalLength);
