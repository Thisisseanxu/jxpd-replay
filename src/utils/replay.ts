...
export const ANONYMOUS_REPLAY_ID = 'anon-jxpd-v1';
  ...
  while (position < data.length) {
    ...
    if (wireType === 0) {
      ...
      if (position + 8 > data.length)
        throw new Error('回放 fixed64 字段被截断');
      fields.push({
        number,
        wireType,
        value: data.slice(position, position + 8),
      });
      position += 8;
    } else if (wireType === 2) {
      ...
      if (position + 4 > data.length)
        throw new Error('回放 fixed32 字段被截断');
      fields.push({
        number,
        wireType,
        value: data.slice(position, position + 4),
      });
      position += 4;
    } else {
...
function encodeFields(fields: WireField[]) {
  ...
  for (const field of fields) {
    parts.push(
      encodeVarint((BigInt(field.number) << 3n) | BigInt(field.wireType)),
    );
    if (field.wireType === 0) {
...
function looksLikePlayer(fields: WireField[]) {
  const hasPlayerIdentity =
    getField(fields, 1)?.wireType === 1 && getField(fields, 2)?.wireType === 2;
  ...
  return !roomMarkers.some(
    ([number, wireType]) => getField(fields, number)?.wireType === wireType,
  );
}
...
function collectPlayer(
  ...
  if (current) {
    if (!current.originalName && originalName)
...
function collectPlayersFromRoom(
  data: Uint8Array,
  players: Map<string, PlayerRecord>,
) {
...
function readReplay(data: Uint8Array) {
  ...
  while (position < data.length) {
    ...
    frames.push({
      commandId,
      payload: data.slice(position, position + payloadLength),
    });
...
export function detectReplayPrivacy(
  ...
  if (marker || hasAnonymousPlayerIdentity(analysis)) {
    ...
    const hasPreservedRoomName = Boolean(
      analysis.roomName && analysis.roomName !== 'match',
    );
...
function transformPlayer(data: Uint8Array, context: TransformContext) {
  ...
  const output = fields.flatMap((field) => {
    ...
    if (field.number === 2 && field.wireType === 2) {
      const nextName =
        shouldReveal && record?.originalName
          ? record.originalName
          : (record?.label ?? 'Player');
...
function remapPackedFixed64(value: Uint8Array, context: TransformContext) {
  ...
  for (let offset = 0; offset < value.length; offset += 8) {
    const anonymousId = context.playerIds.get(
      fixed64(value.slice(offset, offset + 8)).toString(),
    );
    if (anonymousId === undefined) continue;
    output ??= value.slice();
    new DataView(output.buffer, output.byteOffset + offset, 8).setBigUint64(
      0,
      anonymousId,
      true,
    );
...
export function anonymizeReplay(
  analysis: ReplayAnalysis,
  revealPlayers: Set<number>,
  preserveRoomName: boolean,
) {
  const context: TransformContext = {
    players: new Map(
      analysis.players.map((player) => [player.id.toString(), player]),
    ),
    playerIds: anonymousPlayerIds(analysis.players),
    revealPlayers,
    preserveRoomName,
    replayId: ANONYMOUS_REPLAY_ID,
    changedFields: 0,
  };
    ...
    if (frame.commandId === 1003 || frame.commandId === 1113) {
      ...
      const output = fields.map((field) =>
        field.number === roomNumber && field.wireType === 2
          ? {
              ...field,
              value: transformRoom(field.value as Uint8Array, context),
            }
          : field,
      );
      ...
      const fields = parseFields(payload);
      const output = fields.map((field) =>
        field.number === 11 && field.wireType === 2
          ? withText(field, context.replayId)
          : field,
      );
      payload = encodeFields(output);
    }
